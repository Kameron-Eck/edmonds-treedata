...
| E-5 | Low | **Rebase evidence copy is under-tested, and its semantics are unstated.** (a) The copied `stance`/`page` are not checked: the only test uses `supports`, page 1. (b) **The rebase path flattens a chain's evidence onto one version, which promotion never does.** On the promote path each evidence row stays on the version it was attached to, and main's current version carries only its own rows (`use_evidence_status` is keyed by `use_version_id`). On the rebase path, `= ANY (c.version_ids)` copies every chain version's evidence onto the single new head. A quote attached to v1 and dropped by writing v2 comes back on the head: a `refutes` row lands on a version that had none. | X6 (`page`→1, `stance`→`'supports'`): suite passed; probe row became `(1, 'supports', …)` where the original was `(7, 'refutes', …)`. X5 (head evidence only): suite passed. Unmutated probe: head v2 had 0 rows; rebased head has 1 `(7, 'refutes', 4, 20, True)` | Assert every copied column, including a `refutes` row. Kam/design: decide whether rebase should match promotion. If it should, X5 (`= c.head`) is the fix, not a weakening. Either way, write the choice down |
...
37 mutations were not re-run. This was deliberate: harness rows M4a and M4b run `GRANT litkb_writer TO litkb_test WITH INHERIT
TRUE` and `GRANT CONNECT ON DATABASE litkb TO litkb_test` as postgres. Those are changes to the privileges on `litkb`, which
this brief forbids; `litkb` was not written; provisioning was not re-run; the server log was not read.
