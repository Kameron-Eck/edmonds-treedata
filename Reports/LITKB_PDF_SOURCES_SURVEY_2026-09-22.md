...
| identifier crosswalk yield (`qc/instruments/litkb_acq_probe_crosswalk.py`, round 3, every DOI-bearing work in main asked at OpenAlex, Semantic Scholar and Crossref; litkb's table holds doi for all of them, arxiv for six, and nothing under pmid, pmcid, openalex, s2, isbn) | the fan-out fills openalex for every work, a Semantic Scholar corpus id for most, MAG for most, an arXiv id for 71 works, a PMID for 67, a PMCID for 36, an ISBN for 12, and Crossref carries a `relation` on 22 works (ten `has-preprint`); Crossref answers 404 for 32 of the DOIs (DataCite or other agencies) |
| the same probe joined to the file table: identifiers gained by works that have NO active file today (222 of the 466) | an arXiv id for 36 of them (a free preprint route the base could not see), a PMID for 38, a PMCID for 24, an ISBN for 8 (the book and chapter namespace), a `relation` edge for 11 (six `has-preprint`), a Crossref `link[]` for 200; eight of the no-file DOIs are unknown to Crossref |
