...
| **Healing test 1: fill-side laundering audit** - ~300 cells sampled from the healer OWN output, paired interval reads, null-change control pair, adjudicator sub-checked on lidar/finer imagery; report k/300 beside 0/42 | Kam labelling session; sampler instrument first (CPU) |
| **Healing test 2: gap-length spectrum H(L)** with laundered(L), triples(L), n_eligible, crowns-deleted; median as negative control; mutation-tested | zero labels; instrument on the existing heal outputs |
