...
| **12-epoch stack path** — LANDED `b7ee58c` (Opus workflow, refereed): `qc/instruments/heal_stack_build.py` builds an N-epoch 2 m stack (heal_* masks auto-discovered; refuses to touch the published trend8 cache; parity gate vs it), `--stack/--out` threaded through temporal_heal, heal_vs_gold, heal_gap_spectrum, heal_closing_baseline with every default byte-identical. **10-epoch TRIAL on real data (2017 + 2023 heal masks in; scratch outputs, nothing tracked):** BLIND tier already 0 (was 2 brackets); no-change impossible triples 124 → 220 (the inserted epochs flicker against their neighbours: heal_2017 is cut on a probability cliff, heal_2023 recall 0.865 held-out), healer removes 173 of 220 (58 by canopy, the rest by IGNORE); loss laundered 0 (still no power); plain closing at cap 1 launders 7 of 12 in-interval-eligible losses where the healer launders 0 — the tiers doing what the formula cannot. DEFECT exposed: closing rows read laundered_in_interval 13–14 against n_eligible 12 (count and denominator not the same unit) — refereed before any 12-epoch verdict. heal_2017 vs C-CAP 2021 at the held cut: R 0.796 / P 0.753 beside of_2017's 0.788 / 0.753 — UNDETERMINED; delivered cut two u8 steps from the recall cliff | 12-epoch build once heal_2020 + heal_2022 land (healC); then heal_vs_gold / closing / spectrum / fill-audit rebuilt for the experiment's verdict |
