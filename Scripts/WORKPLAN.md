...
| **HEALER TABLED (Kam, 2026-09-08)** — close-out is: heal_2022 finishes on healC, the 12-epoch stack is built (`heal_stack_build.py`, default out), healC stopped, per-arm notes written to `experiments/heal_infill_2017_2023.yaml` (2017 WEAK_CALIBRATION; 2020 AE11 clean; 2023 BE12 clean; 2022 as it lands), harvests + landed. NO further healer analysis: the experiment's verdict (heal_vs_gold / closing / spectrum / fill-audit on 12 epochs, the reader session, gamma_conditional) waits until the system work below is done, so that work moves faster when it resumes | Kam: when to resume |
