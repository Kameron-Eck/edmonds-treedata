...
| **3.5** | `core.py` split | **scoped 2026-08-30, not executed.** Now 2,833 lines / 61 top-level defs. Natural clusters, measured: steps 1,083 · ckpt-select 428 · splits 222 · data+aug 219 · losses 196 · model 187 · staging 111 · unassigned 154. Only ONE name (`__init__`) is ambiguous across the package, so a facade re-export from `core` makes the move invisible to ~30 `core.X` call sites. **Precondition now met:** four gates asserted on the engine's text via a hardcoded `core.py` path and would have passed VACUOUSLY after a split — they are re-anchored on symbols (`names.find_symbol_source`), with a test banning the shape. **Why not executed, and the reason CHANGED on 2026-08-31:** the scoping above was incomplete. It checked call sites (~30, all covered by a facade re-export) and symbol ambiguity (one name) but missed the load-bearing coupling: `_ensure_torch()` binds torch/nn/smp/albumentations into **core.py-the-module's globals** via `global torch, nn, ...`. A moved-out module would have no `torch` at all, and cannot import it at module level because the lazy-torch property is gated twice — `test_ci_gates::test_torch_stays_lazy`, and preflight's undefined-name sweep, which carries an explicit lazy-torch allowlist because those names are undefined at module level by design. That property is what lets the orchestrator survive a broken engine environment, so it is not negotiable. **AND THAT REASON WAS ALSO WRONG, checked 2026-08-31.** Laziness is gated ONCE, not twice: `phase4seg_preflight.py:152` merely PRINTS `torch stayed unloaded: {bool}` and asserts nothing, and its TORCH allowlist is a permission, not a prohibition. Only `test_ci_gates::test_torch_stays_lazy` enforces it. And no split module needs a torch handle at IMPORT time — the handles are used inside function bodies. So the expensive `ensure_torch(globals())` rework is unnecessary and should be REJECTED on design grounds: it imposes a per-module call-ordering obligation whose failure mode is a NameError at call time, in training. **Use function-local `import torch`, exactly as `core.py::sdm_for_mask` already does for scipy.** Start with the losses cluster only: one torch handle, zero monkeypatch surface, a 15-test local suite running real torch. `"losses"` must be added to `phase4seg_preflight.py::MODULES` in the SAME commit — that list is hardcoded and hand-maintained while the symbol locators auto-discover, so a new module otherwise gets zero static gating and nothing says so. A green losses split does NOT board the steps or ckpt-select clusters: those carry monkeypatch surface (`test_select_smooth.py` patches `core._train_one_epoch`) and a test that hardcodes `step_evaluate` living in core.py. |
...
| **4.5** | degradation synthesis — **UNBLOCKED (Kam, 2026-08-31)** | **next.** Phase A: the degradation tool — 2020 imagery + gold labels pushed to each delivery's look via the INVERSE of the measured R2 radiometry table plus GSD resampling, so the transforms are measured and the masks are perfect. Run the blur/downsample/noise/JPEG chain TWICE with randomized params and generalized Gaussian kernels; one clean chain is too regular to transfer. ~1 day to build, then one sigma-judged A/B on a weak early year (~1 A100-h). Phase B copy-paste compositing; Phase C generative ONLY if a gap remains. **Do the free diagnostic first** — see 4.6. |
| **4.6** | the object-ratio diagnostic | **do first, zero GPU, already approved.** Fixed-PIXEL tiles make the tree-to-tile object ratio vary ~7x across tiers (512 px is ~77 m at 15 cm and ~512 m at 1 m). It was logged as a candidate explanation for coarse-year UNDERperformance — and the pilot measured the opposite direction, coarse beating medium on the same date, while the ERF put 50% of influence inside 106 px of a 512 px tile, smaller than one crown at fine GSD. Those three facts are about the same quantity and currently disagree. Cheapest real question on the board. |
| **4.3** | DeepLabV3+ as an arm | **BLOCKED — contradicts a recorded decision.** "Decisions taken (Kam, 2026-08-30)" in this same file reads: *keep the U-Net and resnet101; change the loss, not the backbone.* Surfacing, not adjudicating. Independently, the item's stated justification is wrong: MEASURED on the installed smp 0.5.0, THREE architectures expose `encoder_output_stride` (DeepLabV3, DeepLabV3Plus, PAN), not "the only" one. And `phase4/qc/erf_report.md` — this project's own ERF protocol — already concluded the cheap form is a global-average-pool branch, NOT large-rate atrous taps. If Kam wants global context, the alternative to put to him is a ParseNet-style global-pool branch on the existing U-Net bottleneck: ~0 params, full 2020 warm start intact, zero re-tile. Precondition for ANY non-U-Net run: neither the checkpoint payload nor the run manifest carries an architecture key, so a DeepLabV3+ checkpoint would be indistinguishable from a U-Net one to every reader. |
| **4.4** | resolution effect — reframed 2026-08-31 | **the boarded version is struck.** "+9.2 OA" appears exactly ONCE in this repo: in the sentence that asserted it. No measurement, no literature row, no source — a manufactured-looking figure of the kind `colab_rates.csv` exists to refuse. "Matching training GSD to deployment GSD" also describes a mismatch this pipeline does not have: rule 3.7 deploys every year at its OWN native resolution, so the two already match per year. And "a tiling parameter, not a retrain" is backwards — tiles ARE the training input, so changing them forces a retrain and inference must be decimated to match or the change CREATES the mismatch it claims to remove. **What survives:** the pilot measured coarse (eff 82.5 cm) beating medium (eff 42.6 cm) on the same date, and the ERF put 50% of influence inside 106 px of a 512 px tile. The honest next step is a zero-GPU support-matched rescore of the prob rasters that already exist — one common grid, cells at 0.6/1.0/2.0 m, 255 masked first, aggregating the thresholded call as per-cell area fraction. If the gap collapses toward 1 m it was a measurement-support artifact and resolution is UNDETERMINED; if it survives it is still confounded by program/sensor, and only a within-acquisition test settles it. Keep the item's one surviving half: do NOT super-resolve the coarse end. |
| **5** | pilot slice — 2019 / 2019s / 2019n | **LAUNCHED 2026-08-31 00:44-00:53Z**, three parallel arms, one queue file each (`pipeline/pilot_2019_{fine,medium,coarse}.yaml`). `pilotfine` + `pilotmed` on A100; `pilotcoarse` on **L4** because A100 assignment is CONCURRENCY-capped at 2 for this account (TooManyAssignments x6, not scarcity — see COLAB_AUTONOMY_SETUP.md). All three bootstrapped with ZERO engine diff between their commits, so the 2019s-vs-2019n pair is uncontaminated by code. **CORRECTION to the plan's framing:** that pair is NOT a "same-flight" pair. Both were flown 2019-10-11, but by different programs — Snohomish County HXIP (`2019s`) and USDA NAIP (`2019n`) — so camera, processing and radiometry differ alongside resolution. Same DATE, not same flight; season and phenology are frozen, sensor is not. **3/3 GATE PASS as of 04:36Z** (every arm: mask GPKG, independent score, manifest with EPOCH, and all six steps OK unattended). The coarse runtime died TWICE and was resumed from the ledger both times, inheriting a verified checkpoint rather than redoing ~89 min. **The pilot gate is MET** — the go-condition for the 36-year run, which needs Kam. Numbers live in `phase4/qc/qc_indep_report.csv` (`live=1` rows) — not restated here. Run `py -3.12 qc/pilot_gate.py`. **U3 confirmed on all three tiers**: each ran labels→tile→train→evaluate→inference→postproc with no hand-typed step. |
