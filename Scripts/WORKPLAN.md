...
| **Machinery validation run (bundle_validation_2017k)** — start-up merge 359 s → 5.1/5.9 s, zero in-step installs, zero telemetry loss / torn rows, phases + VERIFY minutes live: all PASS; bundle read 228.2 → 123.7 s but FAIL against its bar, flag stays off | `experiments/bundle_validation_2017k.yaml` verdict (referee-scored); `phase4/qc/{timing_events,hw_step_attribution,runtime_sessions}.csv` |
| **Literature: healing analogues across ten ML fields** (72 mechanisms, 26 skeptic-adjudicated): NO field has a measured laundering rate for a one-directional temporal fill; the convergent fill-odds formula needs P(regain after a recent removal), which the endpoint panel cannot supply; posterior-as-licence REFUSED until it is measured | experiments/lit_healing_analogues.yaml verdict; Reports/LIT_HEALING_ANALOGUES_2026-09-08.md; lit_healing_analogues_2026-09-08.csv |
...
| **One-shot entry point** | `py -3.12 -m litkb hunt <doi-or-url>` / MCP `litkb_hunt` (resolve → admit → bind → extract → ingest → state); proven on a 62-page web PDF in 371 s, idempotent. Web sources land as manual proposals: out of search until a second session approves. A DOI hunt stops at `held` until a spend rule is set |
| **Next phase (designed, not built — needs budget)** | the closed loop: Sonnet review → per-study drop-off (identifiers + expected claim + why + passage) → `litkb_hunt` → circle-back verified use + markdown → Opus vet → a per-workstream BRIEF for the orchestrating model. Missing pieces: the drop-off record (`hunt_request`), the brief export. Also: ligature normaliser (ff/fi/fl → C0 codes break search), pix2tex as a second formula decoder, citation graph anchoring to blocks |
...
| **Tile-bundle handoff** — landed `dd71417`, OFF by default: the validation read staged 1,264 files in 123.7 s (vs 228.2 per-file) but missed its 60 s bar — a ≥86 s zero-traffic stall on a bare copyfile with no timeout or slow-path escape (`experiments/bundle_validation_2017k.yaml` R1) | probe done 05:28Z: same bundle 10.6 s copy / 18.0 s end-to-end hours later, but a chunked re-read stalled 33 s once while the aged control read clean (`phase4/qc/probe_bundle_read_20260908T052818Z.txt`) — stalls are intermittent, not freshness; NEXT: bounded read with a floor + rclone escape, then re-validate R1 |
| **Ortho scratch cache** — LANDED `83d1169` (pinned, journaled, flock-guarded entries; prob raster adopted after its verified copy); saving UNVALIDATED until a same-VM tile+inference job shows the inference `stage <ortho>` row gone from `timing_events.csv` | first same-VM campaign (heal_infill queue is ready) |
| **Per-epoch train profiler** — LANDED `83d1169`: five harvestable rows per epoch (data / gpu sync-at-epoch-end / val / save / other) + evaluate spans, BENCH MATCH; the `save` bucket is still one number (torch.save / sha256 read-back / publish / Drive verify) | split `save` into its four stages next; read the first campaign's rows |
| **Honest-results harvester** — LANDED `1279019`: `qc/instruments/harvest_qc_indep.py` is the ONLY writer of the tracked `qc_indep_report.csv` + per-year breakouts (landed.py rung 7); refuses a shrinking lake table and a re-shaped header, reports double-live lineages and lake-orphaned tracked files. The tracked table had been hand-copied and sat 285 rows / 9 days behind the lake — zero rows for any current-recipe arm while the lake held 42 live | — |
| `common.py::_publish_replace` aside-rename guard — LANDED `83d1169`, mutation-tested three ways | — |
| **Healer vs plain closing at equal cap** — the tier-matched closing reproduces the healer's pre-floor candidate set to the cell (449,663) because every alignment shift rounds to zero 2 m cells; healer − closing = the 28 m² floor (−28.9% cells, −10 triple fixes); whether the floor is a gain is UNDETERMINED (`phase4/qc/heal_closing_baseline.csv`) | decide after the reader session; the landmark transform is inert on the 2 m grid |
...
| **Untimed checkpoint write** — saving epochs cost 58 s vs 24 s non-saving; the timed copy is 2.5 s of a ~34 s tax per save (torch.save, sha256 read-back through the mount, publish, Drive verify all outside the timer); 11–13 min per train step with the A100 idle (`bundle_validation_2017k.yaml` (ii)) | ticks around each stage first (the engine workflow's profiler lands them), then decide what moves off the mount |
...
| **Healing test 1: fill-side laundering audit** — DRAWN (`phase4/qc/heal_fill_audit_sample.csv`: 300 fill components + 300 controls, 220 lidar-adjudicable; design note beside it) | Kam reader session; the fill-side bound at k=0 is 0.99% |
| **Healing test 2: gap-length spectrum** — DONE (`phase4/qc/heal_gap_spectrum.csv`): laundered_at_risk = 0 in every bucket, so the 0-of-42 kill has NO POWER; campaign rule amended on the record (`22d3a9c`) | — |
| **Healing test 3: gamma_conditional** (regain after a recent removal) from adjudicated impossible triples at the no-change points - the 42 losses cannot yield it | small Kam adjudication + instrument; gates every posterior-as-licence idea |
| **12-epoch stack path** — LANDED `b7ee58c` (Opus workflow, refereed): `qc/instruments/heal_stack_build.py` builds an N-epoch 2 m stack (heal_* masks auto-discovered; refuses to touch the published trend8 cache; parity gate vs it), `--stack/--out` threaded through temporal_heal, heal_vs_gold, heal_gap_spectrum, heal_closing_baseline with every default byte-identical. **10-epoch TRIAL on real data (2017 + 2023 heal masks in; scratch outputs, nothing tracked):** BLIND tier already 0 (was 2 brackets); no-change impossible triples 124 → 220 (the inserted epochs flicker against their neighbours: heal_2017 is cut on a probability cliff, heal_2023 recall 0.865 held-out), healer removes 173 of 220 (58 by canopy, the rest by IGNORE); loss laundered 0 (still no power); plain closing at cap 1 launders 7 of 12 in-interval-eligible losses where the healer launders 0 — the tiers doing what the formula cannot. DEFECT exposed and FIXED `802a0c8`: closing rows read laundered_in_interval 13–14 against n_eligible 12 because the numerator counted (point, epoch) fill events against a per-point denominator; both per point now, build() refuses any count above its denominator (mutation-tested), corrected rows acq K=2 11/12, K≥3 12/12, healer 0/12; spectrum given `--heal-vs-gold` and an explicit crown SKIP. heal_2017 vs C-CAP 2021 at the held cut: R 0.796 / P 0.753 beside of_2017's 0.788 / 0.753 — UNDETERMINED; delivered cut two u8 steps from the recall cliff | 12-epoch build once heal_2022 lands (healC), then TABLED — see the next row |
| **HEALER TABLED (Kam, 2026-09-08: "any runtime that is almost finished can continue; don't start any new epochs")** — heal_2022 was never started (healC finished heal_2020 at 19:28Z and idled with its command channel lost; stopped), so the campaign closes at 3 of 4 arms and the stack at ELEVEN epochs (`heal_stack_build.py` default out `D:\edmonds-pipeline\heal_stack_2m.npz`: 8 trend8 + heal_2017/2020/2023). heal_2022 stays in the experiment's `arms:`; its queue file is regenerated by `qc/experiment_queue.py` on resumption (a tabled experiment carries none). Per-arm notes in `experiments/heal_infill_2017_2023.yaml` (2017 WEAK_CALIBRATION; 2020 AE11 clean; 2023 BE12 clean; 2022 not run), harvests + landed. NO further healer analysis: the experiment's verdict (heal_vs_gold / closing / spectrum / fill-audit on 12 epochs, the reader session, gamma_conditional) waits until the system work below is done, so that work moves faster when it resumes | Kam: when to resume |
| **HEALER RESUMED + 12-EPOCH READS (2026-09-10 overnight)** — heal_2022 landed (BE25); 12-epoch stack built with parity; all five instruments run → `phase4/qc/*_12ep.*`. Reads: BLIND inside 2016–2024 = 0; laundered/censored 0/42 (no power); no-change triples 240/327 vs 99/124 (count up, share down 6.4 pp — UNDETERMINED on the rule as written); MECHANISM: `tier_for` grants HEAL only with a lidar epoch downstream, so the dense stack writes NO canopy inside the Panel A window — REVIEW/IGNORE only (`experiments/heal_infill_2017_2023.yaml` extra.reads_12ep). Verdict null pending the human fill-audit read (598-unit worksheet, 120 lidar-adjudicable flagged) | Kam: read the worksheet; decide whether to build REVIEW-tier resolution by size trajectory (the hypothesis's second clause) |
| **Harmonization EXP-H1/H2 on resnet18 — DECIDED 2026-09-10** (`experiments/harmonization_h1_h2.yaml`): H1 CONFIRMED (base spread 0.140 vs C-CAP 2021 / 0.154 vs 2016, no kill); **H2 KILLED** — the 2016 structure channel converges the spread (-0.084 / -0.119) but H2-K1 epoch leak fires on both refs, H2-K2 change laundering fires on every year (`phase4/qc/harm_change_laundering.csv`: +0.035…+0.082 extra call-rate rise on lidar-certified 2005→2016 gain cells — it calls 2016 trees in 2006/2011 imagery), K3 same-flight gap unchanged. 2020_in16 scored when its raster mirrors (cannot change the verdict). | Kam: harmonization needs a survey-invariant input or stops; the crown state model (next row) is the time-series path that does not touch training imagery |
| **Crown state model (method 1) — v1 AND v2 KILLED 2026-09-10** (`experiments/crown_state_model.yaml`, `crown_state_model_v2.yaml`): v1 died of an emission unit defect (f ≥ r in 11/12 epochs, constant paths). v2 fixed it (gate shown firing on v1, passing on the fix, 12 paths) and the kills got WORSE: K1 11/11 at-risk losses laundered in the tail because at r≈0.61 four observed absences (39:1) cannot beat the q_loss=0.02 transition cost (49:1); K2 stuck at 111/327 — a ceiling, since 216 of the 327 triples and 60% of the gold are off-crown. Structural: the crown unit cannot be scored against the healer on this gold. Referee files `phase4/qc/crown_state_v2_vs_gold.csv`, `_placebo.csv` | Kam: choose the next unit — pixel-unit model (where the gold lives) and/or crown-level emission rates measured on the on-crown no-change gold (dev-set caveat). Nothing built until decided |
| **Temporal + spatial consistency — BRAINSTORM BRIEF (source of truth, 2026-09-10)** `Reports/TEMPORAL_SPATIAL_CONSISTENCY_BRAINSTORM_2026-09-10.md`: measured constraints (cell unit, lidar never an input, consensus launders change, per-survey eyesight weights) vs the proposed design (three axes of agreement; development as a dated local loss prior from the existing per-year building masks; lidar as teacher: label correction / rate reference / distillation; deep supervision + resolution curriculum as training levers). Kills table; four open questions; cheapest-first order starting with the building-proximity enrichment count on the gold | Kam: answer §6; then step 1 of §7 |
| **BACKBONE DECISION (Kam, 2026-09-10): recipe refinement runs on ResNet-18** — warm-started from `phase4/models/sem_best_2020_base18.pt` (encoder_bases). At the pre-registered matched-cut LOSO score resnet18 is at or above the 101 on all seven check arms, seed spread 0.0069 vs 0.0085, null pair inside the floor; the 101's +0.075 lidar-input effect did not reproduce on either small encoder (its 2016 base 0.669 is suspect-low) — UNRESOLVED until that base is reseeded. Fine-tunes ~25-35% faster; the pipeline stays I/O-bound. `experiments/backbone_sweep.yaml`, `encoder_bases.yaml` | next: the first recipe hypotheses on resnet18; the 101's 2016 reseed |
| **SYSTEM WORK — the class-level fixes, queued in leverage order (Kam, 2026-09-08: "improve the system so work like this can move faster")** — (1) a MEASURED-FILE REGISTRY gated in the suite: every tracked file under `phase4/qc/` names its lake producer, its harvester and its freshness gate, and a file with none FAILS (the honest-results table sat 285 rows behind with no rung; the class, not the instance); (2) a SYNTHETIC REHEARSAL LAKE run by the suite: a tiny stack that is deliberately not eight epochs, tiny masks, tiny gold, on which every instrument must run end to end (the three 10-epoch defects were all "true only at eight epochs"); (3) APPEND-ONLY lake writes for every table a runtime rewrites, harvest-on-land as the standing backup (the ledger erasure class); (4) versioned object storage for the data plane when Kam decides | next session; each of (1)–(2) is about a day |
