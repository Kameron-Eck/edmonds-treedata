...
| **Crown state model (method 1) — v1 KILLED AS RUN 2026-09-10, v2 PRE-REGISTERED** (`experiments/crown_state_model.yaml` verdict; `crown_state_model_v2.yaml`): referee found a unit defect — pixel precision inverted with crown-pooled prevalence put the false-positive rate above recall in 11/12 epochs, collapsing all 222,435 crowns to constant paths (K1 3/11 laundered, K2 111/327 vs healer 240, K3 placebo flat). Not evidence about the method. v2 = pixel-level plug-in + hard f<r gate shown firing on v1 + loud warning, same kills. 60% of gold points are off-crown (crown unit scores 40% of the gold). K4 needs 2016/lidar-referenced delivered-cut rows for the stack tags (CPU scoring job) | v2 implementer running → referee → verdict; Kam: pixel-unit follow-on if v2 promotes |
