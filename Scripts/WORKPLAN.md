...
| **Tile-bundle handoff** — landed `dd71417`, OFF by default: the validation read staged 1,264 files in 123.7 s (vs 228.2 per-file) but missed its 60 s bar — a ≥86 s zero-traffic stall on a bare copyfile with no timeout or slow-path escape (`experiments/bundle_validation_2017k.yaml` R1) | re-stage the aged bundle on a fresh A100 with the trace (~1 min GPU) to split window from structural; then a chunked copy with a floor and an rclone escape |
