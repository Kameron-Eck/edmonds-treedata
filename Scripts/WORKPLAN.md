...
| **4.5** | degradation synthesis — **UNBLOCKED (Kam, 2026-08-31)** | **inputs verified 2026-08-31; the plan's description of them was wrong THREE ways, and the work survives all three.** (1) There is no "R2 radiometry table" — `R2` appears nowhere in IMAGERY_FACTS or CHATLOG. The real artifact is **R1**: `phase4/qc/radiometry_norm.csv`, 118 rows over **36 acquisitions**, per-band `gain`+`offset`, OLS. That IS a linear transform and IS directly invertible, so the premise holds under the right name. (2) **The reference is `2020s`, not `2020`** (36 acquisitions referenced to it; 10 to `2019s`). The 2020 ANCHOR appears in the table as a *transformed* acquisition, so synthesis should source from 2020s or compose 2020→2020s→Y — sourcing 2020 and applying a 2020s-referenced transform would be silently wrong. (3) **Every fit uses exactly `n_targets = 2`.** An OLS line through two points has zero residual degrees of freedom: it is an exact solution, not a fit, and `fit_quality` (median 16.9, max 33.2 DN) is measuring something else. "Measured transforms" is true; "well-determined" is not. **And "perfect masks" is false** — there are no gold hand labels. `phase3/edmonds_canopy_mask_2020.tif` is a model PREDICTION (CLAUDE.md), and `polygons/` was overwritten with accept-all test data with the 14,476-crown review never finished. Synthetic masks inherit the 2020 base model's blind spots, deciduous marsh included. **Phase A is still worth building** on the corrected framing: measured-but-thin radiometry, model-derived masks, PSF/blur approximate. Chain blur/downsample/noise/JPEG TWICE with randomized params; one clean chain transfers poorly. Then one sigma-judged A/B on a weak early year (~1 A100-h, inside the 2-runtime cap). |
