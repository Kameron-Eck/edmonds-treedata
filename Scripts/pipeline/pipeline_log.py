...
LOG LOCATION:
    treedata/phase4/logs/{script}_{step}_{YYYY-MM-DDTHH-MM}.log
    (logs are run artifacts = data plane; moved from Scripts/logs 2026-08-20 when
    the code left Drive for the D: git repo. Old logs stay in the frozen
    Drive Scripts/logs/.)
...
def _script_sources(script: str, logs_dir: Path):
    ...
    Looks beside THIS file first (the code dir — logs_dir is data-plane since
    2026-08-20 and says nothing about where code lives), then beside __main__.
    """
    cands = []
    code_dir = Path(__file__).resolve().parent
    cands.append(code_dir / f"{script}.py")
...
class StepLogger:
    ...
    logs_dir : Path
        Drive path to the logs/ directory, e.g. BASE / "phase4" / "logs".
    capture_stdout : bool
