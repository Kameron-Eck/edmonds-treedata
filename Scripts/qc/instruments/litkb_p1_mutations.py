...
MUTATIONS = [
    ...
    dict(id="D5b", kill="(D-5) add_evidence: workstream must be open",
         ...
         kind="block", file=f"{MIG}/0010_referee2_decisions.sql",
    ...
    # K4a/K4b, X7 and E4a-E4e target 0014 since the P2 referee's fixes (D7): 0014 REPLACES the 0009 label
    # constraints, so the 0009 text is dead code now. Before that: 0009 REPLACED the 0008 constraint, so a
    ...
    dict(id="K4a", kill="(D-4) another agent in the SAME session cannot approve (referee R1)",
         ...
         kind="replace", file=f"{MIG}/0014_referee_p2_fixes.sql",
         old="\n      AND litkb.norm_label(approver_session) <> litkb.norm_label(admitter_session)));",
         new="));",
         what="drop the approver session <> admitter session clause (live constraint, 0014)"),
    dict(id="K4b", kill="(D-4) the same agent name in ANOTHER session may approve (referee R1b)",
         ...
         kind="replace", file=f"{MIG}/0014_referee_p2_fixes.sql",
         old="<> litkb.norm_label(admitter_session)));",
         new="<> litkb.norm_label(admitter_session) AND approver_agent <> admitter_agent));",
         what="re-add the agent-name clause (the pre-0008 AND form, live constraint 0014)"),
    ...
    # X5/X6 (E-5) are in the 0010 section below, after Kam's decision.
    # 0010 REPLACES add_evidence (0007) and promote_rebase (0008): the rows D5b-D5e, X1, K2a-K2h,
    # X2, X3, X4a, X4b were re-pointed at 0010, where the same strings and markers now live.
    dict(id="X1", kill="(E-6) add_evidence waits for a prepare in flight (FOR SHARE)",
         ...
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
    ...
    dict(id="X7", kill="(E-4) a NULL approver session is refused",
         ...
         what="drop approver_session IS NOT NULL (live constraint, 0014)"),
    ...
    dict(id="E4a", kill="(E-4) sessions compare trimmed",
         ...
         kind="replace", file=f"{MIG}/0014_referee_p2_fixes.sql",
         old="litkb.norm_label(approver_session) <> litkb.norm_label(admitter_session)",
    ...
    dict(id="E4b", kill="(E-4) a blank approver agent is refused",
         ...
         kind="replace", file=f"{MIG}/0014_referee_p2_fixes.sql",
         old="      AND litkb.norm_label(approver_agent) <> ''\n", new="",
    ...
    dict(id="E4c", kill="(E-4) a blank approver session is refused",
         ...
         kind="replace", file=f"{MIG}/0014_referee_p2_fixes.sql",
         old="      AND litkb.norm_label(approver_session) <> ''\n", new="",
    ...
    dict(id="E4d", kill="(E-4) a blank admitter session is refused",
         ...
         kind="block", file=f"{MIG}/0014_referee_p2_fixes.sql",
         marker="guard: admitter labels non-blank after invisible characters are removed",
    ...
    dict(id="X5", kill="(E-5) a rebase copies only the head version's evidence",
         test="test_rebase_copies_only_the_head_versions_evidence",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="e.use_version_id = c.head", new="e.use_version_id = ANY (c.version_ids)",
         what="copy every chain version's evidence (the pre-decision flattening)"),
    dict(id="X6", kill="(E-5) copied page and stance equal the source",
         test="test_rebase_carries_evidence",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="SELECT v_new, e.block_id, e.run_id, e.page, e.quote, e.char_start, e.char_end, e.stance",
         new="SELECT v_new, e.block_id, e.run_id, 1, e.quote, e.char_start, e.char_end, 'supports'",
         what="copy page as 1 and stance as 'supports' (referee X6)"),
    dict(id="E5a", kill="(E-5) copied offsets equal the source",
         test="test_rebase_carries_evidence",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="SELECT v_new, e.block_id, e.run_id, e.page, e.quote, e.char_start, e.char_end, e.stance",
         new="SELECT v_new, e.block_id, e.run_id, e.page, e.quote, e.char_start + 1, e.char_end, e.stance",
         what="shift the copied char_start by one"),
    dict(id="T1", kill="(token) a NULL token is refused",
         test="test_every_workstream_write_requires_its_token[write_proposal-missing]",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="IF NOT coalesce(v_ok, false) THEN", new="IF NOT v_ok THEN",
         what="let a NULL comparison through (IF NOT NULL does not raise)"),
    dict(id="T2", kill="(token) write_proposal presents the token",
         test="test_every_workstream_write_requires_its_token[write_proposal-another_workstreams_token]",
         kind="block", file=f"{MIG}/0010_referee2_decisions.sql",
         marker="guard: write_proposal presents the workstream token",
         what="remove the token check in write_proposal"),
    dict(id="T3", kill="(token) write_fact presents the token",
         test="test_every_workstream_write_requires_its_token[write_fact-another_workstreams_token]",
         kind="block", file=f"{MIG}/0010_referee2_decisions.sql",
         marker="guard: write_fact presents the workstream token",
         what="remove the token check in write_fact"),
    dict(id="T4", kill="(token) add_evidence presents the token (the referee's cross-session insert)",
         test="test_referee_cross_session_evidence_insert_is_refused",
         kind="block", file=f"{MIG}/0010_referee2_decisions.sql",
         marker="guard: add_evidence presents the workstream token",
         what="remove the token check in add_evidence"),
    dict(id="T5", kill="(token) abandon_workstream presents the token",
         test="test_every_workstream_write_requires_its_token[abandon_workstream-another_workstreams_token]",
         kind="block", file=f"{MIG}/0012_referee3_fixes.sql",   # 0012 REPLACES abandon_workstream
         marker="guard: abandon_workstream presents the workstream token",
         what="remove the token check in abandon_workstream (live body, 0012)"),
    dict(id="T6", kill="(token) only the token's hash is stored",
         test="test_open_workstream_returns_a_token_stored_only_as_its_hash",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="VALUES (workstream_id, encode(sha256(convert_to(token, 'UTF8')), 'hex'));",
         new="VALUES (workstream_id, token);",
         what="store the token itself"),
    dict(id="T7", kill="(token) no agent role reads token hashes",
         test="test_open_workstream_returns_a_token_stored_only_as_its_hash",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="-- end of 0010\n",
         new="GRANT SELECT ON litkb.workstream_tokens TO litkb_writer;\n-- end of 0010\n",
         what="grant the writer SELECT on workstream_tokens"),
    dict(id="T8", kill="(token) the token-less write_proposal overload is dropped",
         test="test_token_functions_have_exactly_one_signature",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="DROP FUNCTION litkb.write_proposal(text, uuid, jsonb, uuid, jsonb, text, uuid, text, text);\n",
         new="",
         what="keep the old write_proposal signature (an overload)"),
    dict(id="T9", kill="(token) the token file is git-ignored under Scripts/",
         test="test_workstream_token_file_is_git_ignored",
         kind="replace", file="../.gitignore",
         old=".litkb-workstream\r\n", new="",   # the working copy of .gitignore is CRLF
         what="remove the .litkb-workstream ignore line"),
    dict(id="I1", kill="(ingest) the writer cannot insert an extraction run (referee gap 1, step 1)",
         test="test_writer_cannot_install_a_run_or_make_it_current",
         kind="block", file=f"{MIG}/0010_referee2_decisions.sql",
         marker="guard: writer has no INSERT on extraction tables",
         what="keep the writer's INSERT on the extraction tables"),
    dict(id="I2", kill="(ingest) the writer cannot call set_current_run (referee gap 1, step 2)",
         test="test_writer_cannot_install_a_run_or_make_it_current",
         kind="block", file=f"{MIG}/0010_referee2_decisions.sql",
         marker="guard: writer cannot move a file's current run",
         what="keep the writer's EXECUTE on set_current_run"),
    dict(id="I3", kill="(ingest) the ingest login inserts extraction runs",
         test="test_ingest_installs_a_run_and_makes_it_current",
         kind="block", file=f"{MIG}/0010_referee2_decisions.sql",
         marker="guard: ingest inserts extraction tables",
         what="remove the ingest INSERT grant"),
    dict(id="I4", kill="(ingest) the ingest login moves a file's current run",
         test="test_ingest_installs_a_run_and_makes_it_current",
         kind="block", file=f"{MIG}/0010_referee2_decisions.sql",
         marker="guard: ingest moves a file's current run",
         what="remove the ingest EXECUTE on set_current_run"),
    dict(id="I5", kill="(ingest) the ingest login cannot write proposals",
         test="test_ingest_cannot_write_knowledge",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="-- end of 0010\n",
         new=("GRANT EXECUTE ON FUNCTION litkb.write_proposal(text, uuid, jsonb, uuid, jsonb, text, uuid, text, text, text) "
              "TO litkb_ingest;\n-- end of 0010\n"),
         what="grant EXECUTE on write_proposal to litkb_ingest"),
    dict(id="I6", kill="(ingest) the shared connect() refuses the ingest login",
         test="test_connect_refuses_the_ingest_login",
         kind="block", file="pipeline/litkb/db/connect.py",
    ...
    *[dict(id=f"W{i}", kill=f"(0011) the writer has no direct INSERT on {t}",
    ...
    # W9 and W10 target 0012 since the third referee's fixes: 0012 REPLACES add_use_embedding, so the 0011
    # body is dead code (the same marker now lives in 0012).
    *[dict(id=f"W{i}", kill=f"(0011) {fn} presents the workstream token",
           ...
           kind="block",
           file=f"{MIG}/0012_referee3_fixes.sql" if fn == "add_use_embedding" else f"{MIG}/0011_token_bypass_closed.sql",
    ...
    dict(id="W10", kill="(0011) an embedding goes only onto the named workstream's own use version",
         ...
         kind="block", file=f"{MIG}/0012_referee3_fixes.sql",
         marker="guard: embedding version belongs to the workstream",
         what="remove the ownership check in add_use_embedding (live body, 0012)"),
    ...
    # ── the third referee's mutations (Reports/LITKB_P1_REFEREE3_2026-09-13.md), exact strings ──
    # Where the referee's table elides with "…", the full statement used is written out here: Z5's and Z7's
    # GRANT names the created relation, Z10 names both full signatures, Z12 joins blocks and files.
    dict(id="Z1", kill="(F-4) the token is random, not derived from the slug",
         test="test_workstream_token_is_not_derived_from_the_slug_or_id",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="token := replace(gen_random_uuid()::text || gen_random_uuid()::text, '-', '');",
         new="token := encode(sha256(convert_to(p_slug, 'UTF8')), 'hex');",
         what="token = sha256 of the slug (readable by every role)"),
    dict(id="Z2", kill="(F-4) an empty token is refused",
         test="test_every_workstream_write_requires_its_token[write_proposal-empty]",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="IF NOT coalesce(v_ok, false) THEN",
         new="IF coalesce(p_token, 'x') <> '' AND NOT coalesce(v_ok, false) THEN",
         what="accept '' as a token"),
    dict(id="Z5", kill="(F-5) no agent role reads token hashes through a view",
         test="test_no_agent_role_reads_token_hashes_through_any_relation",
         kind="replace", file=f"{MIG}/0011_token_bypass_closed.sql",
         old="-- end of 0011\n",
         new=("CREATE VIEW litkb.workstream_token_hashes AS SELECT * FROM litkb.workstream_tokens; "
              "GRANT SELECT ON litkb.workstream_token_hashes TO litkb_reader, litkb_writer;\n-- end of 0011\n"),
         what="a view over workstream_tokens, SELECT granted to reader and writer"),
    dict(id="Z6", kill="(F-1) catalog guard: workstream_id column without a foreign key",
         test="test_no_agent_role_holds_a_direct_write_on_a_workstream_table",
         kind="replace", file=f"{MIG}/0011_token_bypass_closed.sql",
         old="-- end of 0011\n",
         new=("CREATE TABLE litkb.ws_notes (id uuid PRIMARY KEY DEFAULT uuidv7(), workstream_id uuid NOT NULL, note text); "
              "GRANT INSERT ON litkb.ws_notes TO litkb_writer;\n-- end of 0011\n"),
         what="a no-FK workstream_id table with writer INSERT"),
    dict(id="Z7", kill="(F-1) catalog guard: two foreign-key hops",
         test="test_no_agent_role_holds_a_direct_write_on_a_workstream_table",
         kind="replace", file=f"{MIG}/0011_token_bypass_closed.sql",
         old="-- end of 0011\n",
         new=("CREATE TABLE litkb.evidence_notes (id uuid PRIMARY KEY DEFAULT uuidv7(), evidence_id uuid REFERENCES "
              "litkb.use_evidence (id), note text); GRANT INSERT ON litkb.evidence_notes TO litkb_writer;\n-- end of 0011\n"),
         what="a child of use_evidence with writer INSERT"),
    dict(id="Z8", kill="(F-1) catalog guard: an updatable view",
         test="test_no_agent_role_holds_a_direct_write_on_a_workstream_table",
         kind="replace", file=f"{MIG}/0011_token_bypass_closed.sql",
         old="-- end of 0011\n",
         new=("CREATE VIEW litkb.use_versions_inbox AS SELECT * FROM litkb.use_versions; "
              "GRANT INSERT ON litkb.use_versions_inbox TO litkb_writer;\n-- end of 0011\n"),
         what="an auto-updatable view over use_versions with writer INSERT (forged version, no token)"),
    dict(id="Z9", kill="(F-5) the writer cannot rewrite a block's text",
         test="test_role_privilege_matrix",
         kind="replace", file=f"{MIG}/0011_token_bypass_closed.sql",
         old="-- end of 0011\n",
         new="GRANT UPDATE (text) ON litkb.blocks TO litkb_writer;\n-- end of 0011\n",
         what="column UPDATE on blocks.text for the writer"),
    dict(id="Z10", kill="(F-5) the ingest login cannot call writer functions",
         test="test_role_privilege_matrix",
         kind="replace", file=f"{MIG}/0011_token_bypass_closed.sql",
         old="-- end of 0011\n",
         new=("GRANT EXECUTE ON FUNCTION litkb.add_candidate(uuid, text, text, text, text, uuid, jsonb, text, jsonb, "
              "integer, jsonb), litkb.add_use_embedding(uuid, text, uuid, text, integer, halfvec) TO litkb_ingest;\n"
              "-- end of 0011\n"),
         what="EXECUTE on add_candidate and add_use_embedding for ingest"),
    dict(id="Z12", kill="(F-6) rebase copies head evidence whose run was superseded",
         test="test_rebase_copies_head_evidence_whose_run_was_superseded",
         kind="replace", file=f"{MIG}/0010_referee2_decisions.sql",
         old="FROM use_evidence e WHERE e.use_version_id = c.head ORDER BY e.id;",
         new=("FROM use_evidence e JOIN blocks bb ON bb.id = e.block_id JOIN files ff ON ff.id = bb.file_id "
              "WHERE e.use_version_id = c.head AND e.run_id = ff.current_run_id ORDER BY e.id;"),
         what="copy only evidence anchored in the file's current run"),
    # ── the fixes' own guards (migration 0012, workstream.py, connect.py) ──
    dict(id="F2a", kill="(F-2) abandon refused while a promotion is prepared",
         test="test_abandon_refused_while_promotion_prepared",
         kind="block", file=f"{MIG}/0012_referee3_fixes.sql",
         marker="guard: no abandon while a promotion is prepared",
         what="remove the prepared-promotion check in abandon_workstream"),
    dict(id="F2b", kill="(F-2) abandon waits for a prepare in flight",
         test="test_abandon_waits_for_a_prepare_in_flight_and_is_refused",
         kind="block", file=f"{MIG}/0012_referee3_fixes.sql",
         marker="guard: abandon locks the workstream row",
         what="remove the FOR UPDATE on the workstream row in abandon_workstream"),
    dict(id="F3a", kill="(F-3) add_use_embedding: workstream must be open",
         test="test_add_use_embedding_refuses_a_closed_workstreams_token",
         kind="block", file=f"{MIG}/0012_referee3_fixes.sql",
         marker="guard: embedding workstream open",
         what="remove the open-workstream check in add_use_embedding"),
    dict(id="F3b", kill="(F-3) add_use_embedding waits for a commit in flight (FOR SHARE)",
         test="test_add_use_embedding_waits_for_a_commit_in_flight_and_is_refused",
         kind="replace", file=f"{MIG}/0012_referee3_fixes.sql",
         old="ws.state = 'open' FOR SHARE;", new="ws.state = 'open';",
         what="drop FOR SHARE on the workstream row in add_use_embedding"),
    dict(id="F3c", kill="(F-3) add_use_embedding: version must be proposed",
         test="test_add_use_embedding_refuses_a_promoted_or_prepared_version",
         kind="block", file=f"{MIG}/0012_referee3_fixes.sql",
         marker="guard: embedding version is proposed",
         what="remove the proposed-state check in add_use_embedding"),
    dict(id="F7a", kill="(F-7) a failed token write leaves no workstream",
         test="test_open_workstream_failure_leaves_no_workstream_and_no_file",
         kind="replace", file="pipeline/litkb/workstream.py",
         old="        with conn.transaction():\n", new="        if True:\n",
         what="open the workstream outside a transaction (autocommit)"),
    dict(id="F7b", kill="(F-7) two openers in one directory leave no orphan",
         test="test_open_workstream_race_in_one_directory_leaves_no_orphan",
         kind="replace", file="pipeline/litkb/workstream.py",
         old="os.O_WRONLY | os.O_CREAT | os.O_EXCL", new="os.O_WRONLY | os.O_CREAT | os.O_TRUNC",
         what="claim the token file without O_EXCL"),
    dict(id="F9a", kill="(F-9) connect() refuses the admin logins",
         test="test_connect_refuses_the_admin_logins[postgres]",
         kind="block", file="pipeline/litkb/db/connect.py",
         marker="guard: admin logins only through connect_admin",
         what="remove the admin-login refusal in connect()"),
    dict(id="F9b", kill="(F-9) connect_admin opens only the admin logins",
         test="test_admin_logins_open_only_through_connect_admin",
         kind="block", file="pipeline/litkb/db/connect.py",
         marker="guard: connect_admin opens only the admin logins",
         what="remove connect_admin's login check"),
]
