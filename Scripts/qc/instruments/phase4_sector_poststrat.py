...
ZERO NEW INFERENCE. Every input is already on the lake:
  * the 1-m cover sidecars phase4/qc/sector_campaign/cover1m/cover_1m_{year}[_{tag}].tif written
    by qc/instruments/phase4_sector_series.py (float32 cover fraction per 1-m cell, nodata -1, already
...
qc/instruments/phase4_sector_series.py accumulates each native strip into the 1-m plane with
...
USAGE
  py -3.12 qc/instruments/phase4_sector_poststrat.py                       # every cover1m sidecar
  py -3.12 qc/instruments/phase4_sector_poststrat.py --arms 2013:citywide_rgb 2020s:sectors_v1 2019n:p2nir
"""
import argparse
...
def main():
    ...
    hdr_common = [
        ...
        f"qc/instruments/phase4_sector_poststrat.py ({ver}, covariates_updated="
        f"{aoi.get('covariates_updated', 'UNSTAMPED')})",
        "# read with pandas.read_csv(path, comment='#')",
        "# ZERO new inference: 1-m cover sidecars from qc/instruments/phase4_sector_series.py, thresholded at "
        ...
        "sectors whose cover",
        "#   sidecar is translated (the qc/instruments/phase4_sector_series.py destination-clamp defect, "
    ...
    hdr_reg = hdr_common[:2] + [
        ...
        "qc/instruments/phase4_sector_series.py",
