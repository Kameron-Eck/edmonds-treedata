...
╔══════════════════════════════════════════════════════════════════╗
  ...
  ------------------------------------------------------------------
  R1 (qc/instruments/radiometry_fingerprint.py) MEASURED the problem: over pseudo-invariant
  ...
  * REGISTRATION. The PIF mask is fixed 2020 geometry reprojected into each
  ...
  USAGE
    py -3.12 qc/instruments/radiometry_norm.py                      # fit + validate
    py -3.12 qc/instruments/radiometry_norm.py --step fit
    py -3.12 qc/instruments/radiometry_norm.py --step validate
    py -3.12 qc/instruments/radiometry_norm.py --step compare-methods   # reproduce the table above
    py -3.12 qc/instruments/radiometry_norm.py --reference 2020 --nir-reference 2018s
    py -3.12 qc/instruments/radiometry_norm.py --method theil-sen
...
def read_fingerprint(path: Path = FP_CSV) -> dict:
    ...
    if not Path(path).exists():
        raise FileNotFoundError(
            ...
            f"  run:  py -3.12 qc/instruments/radiometry_fingerprint.py")
    acq: dict = {}
...
def load_table(table_path: Path = OUT_CSV, force: bool = False) -> dict:
    ...
    if not Path(p).exists():
        raise FileNotFoundError(
            ...
            f"  run:  py -3.12 qc/instruments/radiometry_norm.py --step fit")
    table = {}
...
def normalize_stack_band(stack, acquisition, bands=("R", "G", "B", "N"),
    ...
    bands       : band names in stack order. The repo-wide 4-band convention is
                  ...
                  pipeline/make_nir_stack.py and qc/instruments/phase4_qc_ndvi.py.
