...
def test_sweep_arms_are_derived_from_the_benchmark_list():
    """experiments/backbone_sweep.yaml: each non-canary arm is bbNN_<treatment-tag>
    for an encoder in the sweep and a (year, tag) in BENCHMARK_ARMS — every one
    of them, for every encoder."""
    bb = _bb()
    spec = yaml.safe_load((SCRIPTS / "experiments" / "backbone_sweep.yaml")
    ...
    got = {(str(a["year"]), a["tag"]) for a in spec["arms"]}
